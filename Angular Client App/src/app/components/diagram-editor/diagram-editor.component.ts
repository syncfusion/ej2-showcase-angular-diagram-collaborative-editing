...
export class DiagramEditorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  async ngAfterViewInit(): Promise<void> {
    ...
    this._subs.push(
      ...
      this.collab.renderPeerSelectionChanged$.subscribe((selectionEvent) => this._peerSelectionChanged(selectionEvent))
    );
  ...
  private _peerSelectionChanged(selectionEvent: SelectionEvent): void {
    if (!selectionEvent.userId || !selectionEvent.elementIds?.length) {
      if (selectionEvent.connectionId) this.collab.peerSelections.delete(selectionEvent.connectionId);
    } else {
      ...
      this.collab.peerSelections.set(peerConnectionId, {
        userName: selectionEvent.userName || '',
        nodeIds: peerSelectedNodeIds,
        selectorBounds: peerSelectorBounds
      });

      this.collab.peerSelections.forEach((existingPeerSelection, existingPeerConnectionId) => {
        if (existingPeerConnectionId === peerConnectionId) return;
        if (existingPeerSelection.nodeIds && this.collab.setsEqual(existingPeerSelection.nodeIds, peerSelectedNodeIds)) {
          const existingPeerBounds = existingPeerSelection.selectorBounds || null;
          const newPeerBounds = peerSelectorBounds;
          if (!this.collab.boundsEqual(existingPeerBounds?.bounds, newPeerBounds?.bounds) ||
            (existingPeerBounds?.rotationAngle || 0) !== (newPeerBounds?.rotationAngle || 0)) {
            this.collab.peerSelections.set(existingPeerConnectionId, {
      ...
      // If the incoming selection's node IDs appear inside any existing peer selection,
      // recompute that peer's selector bounds from the actual diagram objects and update it.
      this.collab.peerSelections.forEach((existingPeerSelection, existingPeerConnectionId) => {
        if (existingPeerConnectionId === peerConnectionId) { return; }
        if (!existingPeerSelection.nodeIds || existingPeerSelection.nodeIds.size === 0) { return; }
        const diagramInstance: any = this.diagramComponent;

        const hasOverlap = Array.from(existingPeerSelection.nodeIds).some((id) => {
          const obj = (diagramInstance as any).getObject?.(id);
          if (!obj) { return false; }

          // If object is a connector, treat as overlap
          if (obj.sourcePoint && obj.targetPoint) {
            return true;
          }

          // Default: check normal node id overlap
          return peerSelectedNodeIds.has(id);
        });

        // If any of the newly selected ids are contained in this peer's node set
        //const hasOverlap = Array.from(existingPeerSelection.nodeIds).some((id) => peerSelectedNodeIds.has(id));
        ...
        Array.from(existingPeerSelection.nodeIds).forEach((elementId: string) => {
        ...
        if (minLeft === Number.POSITIVE_INFINITY || maxRight === Number.NEGATIVE_INFINITY) {
          // nothing found for this peer selection
          return;
        }

        const width = isFinite(maxRight - minLeft) ? (maxRight - minLeft) : 0;
        const height = isFinite(maxBottom - minTop) ? (maxBottom - minTop) : 0;
        const rotationAngle = existingPeerSelection.selectorBounds?.rotationAngle || 0;
        const computedBounds: SelectorBounds = {
          bounds: {
            x: minLeft + width / 2,
            y: minTop + height / 2,
            width,
        ...
        this.collab.peerSelections.set(existingPeerConnectionId, { userName: existingPeerSelection.userName, nodeIds: existingPeerSelection.nodeIds, selectorBounds: computedBounds });
      });
    }
    this.renderPeerBadges();
  }
  ...
  renderPeerBadges(): void {
    ...
    this.collab.peerSelections.forEach((peer) => {
      ...
      const strokeColor = DiagramFacadeService.getUserColor(peer.userName);
      ...
      if (!isNaN(peerUserId)) {
        ...
        badgeContainer.setAttribute('style', `background:${strokeColor};color:#fff;border-radius:10px;display:flex;align-items:center;justify-content:center;width:50px;height:30px;font-size:12px;`);
        ...
        if (selectedItems && selectedItems.nodes && selectedItems.nodes.length > 0 && firstElementId) {
          ...
          if (peerSelectedNodeIds && peerSelectedNodeIds.size > 1) {
            ...
            Array.from(peerSelectedNodeIds).forEach((elementId: string) => {
              const obj = this.diagramComponent.getObject(elementId) as any;
              if (!obj) return;
