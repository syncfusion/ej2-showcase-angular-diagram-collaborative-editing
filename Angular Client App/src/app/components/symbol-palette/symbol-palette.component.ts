...
import { getFloorPlanShapes } from '../../data/floorplan-shapes';
import { getNetworkShapes } from '../../data/network-shapes';
import { ManagePalettesDialogComponent, PaletteMetadata } from './manage-palettes-dialog/manage-palettes-dialog.component';

@Component({
  selector: 'app-symbol-palette',
  standalone: true,
  imports: [CommonModule, SymbolPaletteModule, ManagePalettesDialogComponent],
...
export class SymbolPaletteComponent implements OnInit {
  ...
  /** Stable reference passed to ejs-symbolpalette — only rebuilt when palette toggles change. */
  activePalettes: PaletteModel[] = [];

  paletteMeta: PaletteMetadata[] = [
    {
      id: 'flow', title: 'Flow Shapes', displayName: 'Flow',
      enabled: true, shapes: this._createFlowShapes()
    },
    {
      id: 'basic', title: 'Basic Shapes', displayName: 'Basic',
      enabled: true, shapes: this._createBasicShapes()
    },
    {
      id: 'bpmn', title: 'BPMN Shapes', displayName: 'BPMN',
      enabled: true, shapes: this._createBPMNShapes()
    },
    {
      id: 'connectors', title: 'Connectors', displayName: 'Connectors',
      enabled: true, shapes: this._createConnectors()
    },
    {
      id: 'network', title: 'Network Shapes', displayName: 'Network',
      enabled: false, shapes: getNetworkShapes()
    },
    {
      id: 'floorplan', title: 'Floorplan Shapes', displayName: 'Floorplan',
      enabled: false, shapes: getFloorPlanShapes()
    },
    {
      id: 'electrical', title: 'Electrical Shapes', displayName: 'Electrical',
      enabled: false, shapes: this._createElectricalShapes()
    }
  ];
  ...
  getNodeDefaults(node: NodeModel): NodeModel {
    if (!(node.addInfo && (node.addInfo as any).type === 'CustomShapes') && (!node.children)) {
  ...
  /** Apply: commit the checked ids emitted by the dialog, then close. */
  onDialogApply(enabledIds: string[]): void {
    this.paletteMeta.forEach((p) => (p.enabled = enabledIds.includes(p.id)));
    this.isDialogOpen = false;
    this._rebuildActivePalettes();
    this.cdr.markForCheck();
  }

  /** Cancel: discard — just close the dialog without touching paletteMeta. */
  onDialogCancel(): void {
    this.isDialogOpen = false;
    this.cdr.markForCheck();
  }

  /** Rebuilds the stable `activePalettes` array. Called once on init and after Apply. */
  private _rebuildActivePalettes(): void {
    this.activePalettes = this.paletteMeta
      .filter((p) => p.enabled)
      .map((p) => ({
        id: p.id,
        title: p.title,
  ...
  private _createFlowShapes(): NodeModel[] {
    return [
  ...
  private _createBasicShapes(): NodeModel[] {
    return [
  ...
  private _createConnectors(): ConnectorModel[] {
    return [
      {
  ...
  private _createBPMNShapes(): NodeModel[] {
    const bpmnShapes: NodeModel[] = [
      ...
      },
      {
        ...
        constraints: NodeConstraints.Default | NodeConstraints.AllowDrop, width: 250, height: 150
      },
